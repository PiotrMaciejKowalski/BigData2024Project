...
# FIXME przenieść do katalogu src / @see (https://app.clickup.com/t/86bwj0nt4)
def plot_time_series(df: pd.DataFrame, atrybut: str) -> None:
  """
  Funkcja generująca wykres w czasie dla zadanego atrybutu.
  Parametry:
  - df (DataFrame): szereg czasowy; Pandas DataFrame
  - atrybut (str): atrybut, dla którego chcemy zrobić wykres w czasie
  """
  # Obliczanie 12-miesięcznej średniej kroczącej
  df['12m_MA'] = df[atrybut].rolling(window=12).mean()
  # Tworzenie wykresu
  ...
  ax1.set_ylabel(f"Wartość {atrybut}")
