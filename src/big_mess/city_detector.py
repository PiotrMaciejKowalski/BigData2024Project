...
class City_Geolocalization_Data:
    def __init__(self, path: str) -> None:
        ...
        self.geodata.rename(
            columns={"miasto": "City", "stan": "State", "panstwo": "Country"},
            inplace=True,
        )
    ...
    def get_cityname(
        ...
        if self.geodata[(self.geodata.lon == lon) & (self.geodata.lat == lat)].empty:
            ...
            return self.geodata[
                (self.geodata.lon == lon) & (self.geodata.lat == lat)
            ].City.iloc[0]
    ...
    def spark_add_geodata(
        ...
        pandas_geodata = self.__pandas_bulk_get_all_geodata(
            city=city, state=state, country=country
        )
        spark_session: SparkSession = sdf.sql_ctx.sparkSession
        spark_geodata = spark_session.createDataFrame(pandas_geodata)
        spark_geodata = spark_geodata.withColumnRenamed(
            "lon", "lon_geo"
        ).withColumnRenamed("lat", "lat_geo")
        return sdf.join(
            spark_geodata,
            [sdf.lat == spark_geodata.lat_geo, sdf.lon == spark_geodata.lon_geo],
            "left",
        ).drop("lon_geo", "lat_geo")
